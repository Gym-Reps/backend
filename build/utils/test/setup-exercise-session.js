...
var require_assertion = __commonJS({
  "node_modules/supertest/lib/cookies/assertion.js"(exports2, module2) {
    "use strict";
    ...
    module2.exports = function(secret, asserts) {
      ...
      function Assertion(res) {
        ...
        if (request2.headers.cookie) {
          const cookies = String(request2.headers.cookie);
          cookies.split(/; */).forEach(function(cookie) {
            request2.cookies.push(Assertion.parse(cookie));
          });
        }
        ...
        assertions.every(function(assertion) {
          result = assertion(request2, response);
...
var require_cookies = __commonJS({
  "node_modules/supertest/lib/cookies/index.js"(exports2, module2) {
    "use strict";
...
// src/utils/test/setup-exercise-session.ts
var setup_exercise_session_exports = {};
__export(setup_exercise_session_exports, {
  setupExerciseSession: () => setupExerciseSession
});
module.exports = __toCommonJS(setup_exercise_session_exports);
...
async function setupExerciseSession(app, token) {
  const auth = `Bearer ${token}`;
  const catalog = await (0, import_supertest.default)(app.server).get("/catalog/exercises?q=squat").set("Authorization", auth).send();
  const exerciseCatalogId = catalog.body.exercises[0].id;
  const templateResponse = await (0, import_supertest.default)(app.server).post("/trainment-templates").set("Authorization", auth).send({ title: "Lower A" });
  const trainmentTemplateId = templateResponse.body.trainmentTemplate.id;
  const exerciseTemplateResponse = await (0, import_supertest.default)(app.server).post(`/trainment-templates/${trainmentTemplateId}/exercises`).set("Authorization", auth).send({ exerciseCatalogId });
  const exerciseTemplateId = exerciseTemplateResponse.body.exerciseTemplate.id;
  const trainmentResponse = await (0, import_supertest.default)(app.server).post("/trainments").set("Authorization", auth).send({ trainmentTemplateId });
  const trainmentId = trainmentResponse.body.trainment.id;
  const exerciseResponse = await (0, import_supertest.default)(app.server).post(`/trainments/${trainmentId}/exercises`).set("Authorization", auth).send({ exerciseTemplateId, plannedSets: PLANNED_SETS });
