...
// src/use-cases/metrics/compute-trainment-metrics/compute-trainment-metrics.spec.ts
...
var InMemoryExercisesRepository = class {
  ...
  async findManyByTrainmentId(trainmentId) {
    return this.items.filter((item) => item.trainment_id === trainmentId).sort((a, b2) => a.created_at.getTime() - b2.created_at.getTime());
  }
  async save(exercise) {
    const index2 = this.items.findIndex((item) => item.id === exercise.id);
    if (index2 >= 0) {
      this.items[index2] = exercise;
    }
    return exercise;
  }
  async delete(id) {
...
var InMemorySetsRepository = class {
  ...
  async createMany(data) {
    const created = data.map((item) => {
      const set3 = {
        id: item.id ?? (0, import_node_crypto3.randomUUID)(),
  ...
  async findManyByExerciseId(exerciseId) {
    return this.items.filter((item) => item.exercise_id === exerciseId).sort((a, b2) => a.index - b2.index);
  }
  async findManyByTrainmentId(trainmentId) {
    return this.items.filter((item) => item.trainment_id === trainmentId).sort((a, b2) => a.index - b2.index);
  }
  async countByExerciseId(exerciseId) {
    return this.items.filter((item) => item.exercise_id === exerciseId).length;
  }
  async save(set3) {
    const index2 = this.items.findIndex((item) => item.id === set3.id);
    if (index2 >= 0) {
      this.items[index2] = set3;
    }
    return set3;
  }
  async delete(id) {
...
var InMemoryTrainmentsRepository = class {
  ...
  async create(data) {
    const trainment = {
      id: data.id ?? (0, import_node_crypto4.randomUUID)(),
  ...
  async findManyByUserId(userId, params) {
    return this.items.filter((item) => {
      ...
      return true;
    }).sort((a, b2) => b2.started_at.getTime() - a.started_at.getTime()).slice((params.page - 1) * PAGE_SIZE, params.page * PAGE_SIZE);
  }
  async findFinishedByUserIdInPeriod(userId, start, end) {
    ...
    ).sort((a, b2) => b2.finished_at.getTime() - a.finished_at.getTime());
  }
  async findPreviousSameTemplate(params) {
    ...
    ).sort((a, b2) => b2.started_at.getTime() - a.started_at.getTime())[0] ?? null;
  }
  async save(trainment) {
    const index2 = this.items.findIndex((item) => item.id === trainment.id);
    if (index2 >= 0) {
      this.items[index2] = trainment;
    }
    return trainment;
  }
};
...
var ComputeTrainmentMetricsUseCase = class {
  constructor(trainmentsRepository2, exercisesRepository2, setsRepository2, metricsRepository2) {
    this.trainmentsRepository = trainmentsRepository2;
    this.exercisesRepository = exercisesRepository2;
  ...
  async execute({
    ...
    for (const exercise of currentExercises) {
      ...
      const previousByIndex = new Map(previousSets.map((set3) => [set3.index, set3]));
      for (const set3 of currentSets) {
        const previousSet = previousByIndex.get(set3.index);
        if (!previousSet) {
          continue;
        }
        if (set3.weight === null || set3.repetitions === null || previousSet.weight === null || previousSet.repetitions === null) {
          continue;
        }
        const metric = await this.metricsRepository.upsertByCurrentSetId({
          ...
          current_set_id: set3.id,
          weight_diff: set3.weight - previousSet.weight,
          repetitions_diff: set3.repetitions - previousSet.repetitions
        });
        metrics.push(metric);
      }
    }
...
// src/use-cases/metrics/compute-trainment-metrics/compute-trainment-metrics.spec.ts
var trainmentsRepository;
var exercisesRepository;
var setsRepository;
var metricsRepository;
var sut;
var USER = "user-1";
var TEMPLATE = "trainment-template-1";
var EXERCISE_TEMPLATE = "exercise-template-1";
async function seedSession(params) {
  const trainment = await trainmentsRepository.create({
    ...
    finished_at: new Date(params.startedAt.getTime() + 60 * 60 * 1e3)
  });
...
describe("Compute Trainment Metrics Use Case", () => {
  ...
  it("computes a +2kg diff across 3 sets between consecutive same-template sessions", async () => {
    await seedSession({
      startedAt: /* @__PURE__ */ new Date("2026-06-24T18:00:00Z"),
      sets: [
    ...
    globalExpect(metrics).toHaveLength(3);
    globalExpect(metrics.every((m) => m.weight_diff === 2)).toBe(true);
    globalExpect(metrics.every((m) => m.repetitions_diff === 0)).toBe(true);
  });
  it("produces a negative repetitions_diff when fewer reps are logged", async () => {
    ...
    const { metrics } = await sut.execute({ trainmentId });
    globalExpect(metrics).toHaveLength(1);
    globalExpect(metrics[0]?.weight_diff).toBe(0);
    globalExpect(metrics[0]?.repetitions_diff).toBe(-3);
  });
  it("yields no metrics for the first-ever session of a template", async () => {
    ...
    const { metrics } = await sut.execute({ trainmentId });
    globalExpect(metrics).toHaveLength(0);
  });
  ...
  it("skips sets with unlogged (null) weight or repetitions", async () => {
    ...
    const { metrics } = await sut.execute({ trainmentId });
    globalExpect(metrics).toHaveLength(1);
    globalExpect(metrics[0]?.weight_diff).toBe(2);
  });
  it("is idempotent: running twice yields the same metrics with no duplicates", async () => {
    ...
    const { metrics } = await sut.execute({ trainmentId });
    globalExpect(metrics).toHaveLength(2);
    globalExpect(metricsRepository.items).toHaveLength(2);
  });
