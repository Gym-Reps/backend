...
// src/use-cases/user-preferences/get-user-preferences/get-user-preferences.spec.ts
...
describe("Get User Preferences Use Case", () => {
  ...
  it("returns the stored preferences for the user", async () => {
    ...
    globalExpect(resolvePreferences(userPreferences.preferences).weightUnit).toEqual(
      "lb"
    );
  });
  it("throws when no preferences row exists", async () => {
    await globalExpect(sut.execute({ userId: "ghost" })).rejects.toBeInstanceOf(
