...
// src/use-cases/user-preferences/create-default-user-preferences/create-default-user-preferences.spec.ts
...
describe("Create Default User Preferences Use Case", () => {
  ...
  it("creates a row with the documented defaults", async () => {
    ...
    globalExpect(userPreferences.user_id).toEqual("user-1");
    globalExpect(value).toEqual({
      weightUnit: "kg",
      theme: "light",
      lengthUnit: "meters",
      weeklyTrainingCount: null
  ...
  it("rejects a second creation for the same user", async () => {
    ...
    await globalExpect(sut.execute({ userId: "user-1" })).rejects.toBeInstanceOf(
