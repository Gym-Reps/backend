...
var UpdateSetUseCase = class {
  ...
  async execute({
    ...
    if (repetitions !== void 0) {
      set3.repetitions = repetitions;
    }
    set3.performed_at = performedAt ?? /* @__PURE__ */ new Date();
    const updated = await this.setsRepository.save(set3);
    return { set: updated };
  }
};

// src/use-cases/sets/update-set/update-set.spec.ts
var setsRepository;
var sut;
async function seedSet(userId = "user-1") {
  const [set3] = await setsRepository.createMany([
    {
...
describe("Update Set Use Case", () => {
  ...
  it("logs actual weight and repetitions and stamps performed_at", async () => {
    ...
    globalExpect(set3.weight).toEqual(82.5);
    globalExpect(set3.repetitions).toEqual(8);
    globalExpect(set3.performed_at).toEqual(globalExpect.any(Date));
  });
