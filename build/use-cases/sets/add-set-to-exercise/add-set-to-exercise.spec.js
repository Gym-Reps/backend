...
var InMemorySetsRepository = class {
  ...
  async createMany(data) {
    const created = data.map((item) => {
      const set3 = {
        id: item.id ?? (0, import_node_crypto2.randomUUID)(),
...
var AddSetToExerciseUseCase = class {
  constructor(setsRepository2, exercisesRepository2, trainmentsRepository2) {
  ...
  async execute({
    ...
    const [set3] = await this.setsRepository.createMany([
      {
...
// src/use-cases/sets/add-set-to-exercise/add-set-to-exercise.spec.ts
var setsRepository;
...
describe("Add Set To Exercise Use Case", () => {
  ...
  it("extends planned_sets and inserts the matching set, keeping the invariant", async () => {
    ...
    const { set: set3 } = await sut.execute({
      userId: "user-1",
      exerciseId: exercise.id
    });
    globalExpect(set3.index).toEqual(3);
    globalExpect(set3.weight).toBeNull();
    const stored = await exercisesRepository.findById(exercise.id);
    const planned = asPlannedSets(stored?.planned_sets);
    globalExpect(plannedSetCount(planned)).toEqual(3);
    globalExpect(planned["3"]).toEqual({ weight: 80, min_reps: 6, max_reps: 12 });
  ...
  it("uses provided placeholders for the new entry", async () => {
    ...
    const stored = await exercisesRepository.findById(exercise.id);
    globalExpect(asPlannedSets(stored?.planned_sets)["3"]).toEqual({
      weight: 100,
      min_reps: 3,
      max_reps: 5
    });
