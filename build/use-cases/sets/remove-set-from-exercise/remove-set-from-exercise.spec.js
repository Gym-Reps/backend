...
var RemoveSetFromExerciseUseCase = class {
  constructor(setsRepository2, exercisesRepository2) {
  ...
  async execute({
    ...
    const set3 = await this.setsRepository.findById(setId);
    if (!set3) {
      throw new ResourceNotFoundError();
    }
    if (set3.user_id !== userId) {
      throw new NotAllowedError();
    }
    const exercise = await this.exercisesRepository.findById(set3.exercise_id);
    if (!exercise) {
      throw new ResourceNotFoundError();
    }
    const plannedSets = { ...asPlannedSets(exercise.planned_sets) };
    const lastIndex = plannedSetCount(plannedSets);
    if (set3.index !== lastIndex) {
      throw new InvalidSetIndexError();
    }
    delete plannedSets[String(lastIndex)];
    exercise.planned_sets = plannedSets;
    await this.setsRepository.delete(set3.id);
    await this.exercisesRepository.save(exercise);
  }
};

// src/use-cases/sets/remove-set-from-exercise/remove-set-from-exercise.spec.ts
var setsRepository;
...
describe("Remove Set From Exercise Use Case", () => {
  ...
  it("removes the last set and its plan key, keeping the invariant", async () => {
    ...
    globalExpect(plannedSetCount(asPlannedSets(stored?.planned_sets))).toEqual(1);
    globalExpect(await setsRepository.countByExerciseId(exercise.id)).toEqual(1);
    globalExpect(await setsRepository.findById(lastSet.id)).toBeNull();
  });
