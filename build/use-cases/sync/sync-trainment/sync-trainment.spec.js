...
// src/use-cases/sync/sync-trainment/sync-trainment.spec.ts
...
var intToChar3 = /* @__PURE__ */ new Uint8Array(64);
var charToInt3 = /* @__PURE__ */ new Uint8Array(128);
...
var InMemoryExerciseTemplatesRepository = class {
  ...
  async save(exerciseTemplate) {
    ...
    if (index2 >= 0) {
      this.items[index2] = exerciseTemplate;
    }
    return exerciseTemplate;
  }
};
...
var InMemoryTrainmentSyncRepository = class {
  ...
  async persistTrainmentGraph(graph) {
    ...
    for (const exercise of graph.exercises) {
      for (const set3 of exercise.sets) {
        const existingSet = this.sets.find((s) => s.id === set3.id);
  ...
  upsertTrainment(trainment) {
    const index2 = this.trainments.findIndex((t) => t.id === trainment.id);
    if (index2 >= 0) {
      this.trainments[index2] = trainment;
    } else {
      this.trainments.push(trainment);
    }
  }
  upsertExercise(exercise) {
    const index2 = this.exercises.findIndex((e) => e.id === exercise.id);
    if (index2 >= 0) {
      this.exercises[index2] = exercise;
    } else {
      this.exercises.push(exercise);
    }
  }
  upsertSet(set3) {
    const index2 = this.sets.findIndex((s) => s.id === set3.id);
    if (index2 >= 0) {
      this.sets[index2] = set3;
    } else {
      this.sets.push(set3);
    }
  }
...
var InMemoryTrainmentTemplatesRepository = class {
  ...
  async create(data) {
    const template = {
      id: data.id ?? (0, import_node_crypto3.randomUUID)(),
  ...
  async save(template) {
    const index2 = this.items.findIndex((item) => item.id === template.id);
    if (index2 >= 0) {
      this.items[index2] = template;
    }
    return template;
  }
};

// src/queues/in-memory/in-memory-event-queue.ts
var InMemoryEventQueue = class {
  jobs = [];
  async add(job) {
    this.jobs.push(job);
  }
...
var SyncTrainmentUseCase = class {
  constructor(trainmentSyncRepository, trainmentTemplatesRepository2, exerciseTemplatesRepository2, eventQueue2) {
    this.trainmentSyncRepository = trainmentSyncRepository;
    this.trainmentTemplatesRepository = trainmentTemplatesRepository2;
    this.exerciseTemplatesRepository = exerciseTemplatesRepository2;
  ...
  assertSetInvariant(exercise) {
    const indices = exercise.sets.map((set3) => set3.index).sort((a, b2) => a - b2);
...
// src/use-cases/sync/sync-trainment/sync-trainment.spec.ts
var syncRepository;
var trainmentTemplatesRepository;
var exerciseTemplatesRepository;
var eventQueue;
var sut;
function buildExercise(exerciseTemplateId, setCount = 2) {
  const plannedSets = Object.fromEntries(
    Array.from({ length: setCount }, (_, i2) => [
      String(i2 + 1),
      { weight: 80, min_reps: 6, max_reps: 12 }
    ])
  );
  const sets = Array.from({ length: setCount }, (_, i2) => ({
  ...
  return { id: (0, import_node_crypto4.randomUUID)(), exerciseTemplateId, plannedSets, sets };
}
...
describe("Sync Trainment Use Case", () => {
  ...
  it("persists a full graph (1 trainment, 2 exercises, N sets), preserving client ids", async () => {
    const { trainmentTemplate, exerciseTemplateA, exerciseTemplateB } = await seedTemplate();
    const trainmentId = (0, import_node_crypto4.randomUUID)();
    const exercises = [
    ...
    const result = await sut.execute({
      ...
      startedAt: /* @__PURE__ */ new Date("2026-06-29T18:00:00Z"),
      finishedAt: /* @__PURE__ */ new Date("2026-06-29T18:50:00Z"),
      exercises
    });
    globalExpect(result.created).toBe(true);
    globalExpect(result.trainment.id).toBe(trainmentId);
    globalExpect(result.exercises).toHaveLength(2);
    globalExpect(result.sets).toHaveLength(5);
    globalExpect(syncRepository.trainments).toHaveLength(1);
    globalExpect(syncRepository.sets).toHaveLength(5);
    globalExpect(syncRepository.events).toHaveLength(1);
    globalExpect(syncRepository.events[0]?.metadata).toEqual({
      trainmentId
    });
    globalExpect(eventQueue.jobs).toHaveLength(1);
    globalExpect(eventQueue.jobs[0]?.eventId).toBe(result.eventId);
    globalExpect(syncRepository.exercises.map((e) => e.id).sort()).toEqual(
  ...
  it("forces user_id from the caller onto every set", async () => {
    ...
    globalExpect(syncRepository.sets.every((s) => s.user_id === "user-1")).toBe(true);
    globalExpect(syncRepository.trainments[0]?.user_id).toBe("user-1");
  });
  it("is atomic: a graph whose 2nd exercise breaks the invariant persists nothing", async () => {
    ...
    globalExpect(syncRepository.trainments).toHaveLength(0);
    globalExpect(syncRepository.exercises).toHaveLength(0);
    globalExpect(syncRepository.sets).toHaveLength(0);
  });
  it("is idempotent: re-syncing the same graph yields no duplicates and returns 200-equivalent", async () => {
    ...
    globalExpect(first.created).toBe(true);
    globalExpect(second.created).toBe(false);
    globalExpect(syncRepository.trainments).toHaveLength(1);
    globalExpect(syncRepository.exercises).toHaveLength(1);
    globalExpect(syncRepository.sets).toHaveLength(3);
  });
  ...
  it("throws ResourceNotFoundError when the template does not exist", async () => {
    await globalExpect(
      sut.execute({
        userId: "user-1",
        id: (0, import_node_crypto4.randomUUID)(),
        trainmentTemplateId: (0, import_node_crypto4.randomUUID)(),
        startedAt: /* @__PURE__ */ new Date(),
        finishedAt: null,
        exercises: []
      })
