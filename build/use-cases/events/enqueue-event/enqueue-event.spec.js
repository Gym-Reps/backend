...
// src/use-cases/events/enqueue-event/enqueue-event.spec.ts
...
describe("Enqueue Event Use Case", () => {
  ...
  it("creates a PENDING event with the given type/metadata and enqueues a job keyed by the event id", async () => {
    ...
    globalExpect(event.status).toBe("PENDING");
    globalExpect(event.event_type).toBe("COMPUTE_TRAINMENT_METRICS");
    globalExpect(event.user_id).toBe("user-1");
    globalExpect(event.metadata).toEqual({ trainmentId: "trainment-1" });
    globalExpect(eventsRepository.items).toHaveLength(1);
    globalExpect(eventQueue.jobs).toHaveLength(1);
    globalExpect(eventQueue.jobs[0]?.eventId).toBe(event.id);
    globalExpect(eventQueue.jobs[0]?.eventType).toBe("COMPUTE_TRAINMENT_METRICS");
  });
