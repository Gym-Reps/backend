...
var ProcessEventUseCase = class {
  constructor(eventsRepository2, handlers) {
  ...
  async execute({
    ...
    const handler2 = this.handlers[event.event_type];
    if (!handler2) {
      throw new Error(
...
// src/use-cases/events/process-event/process-event.spec.ts
var eventsRepository;
var handler;
var sut;
function makeSpyHandler(impl) {
  return {
    calls: 0,
    async handle() {
      this.calls += 1;
...
describe("Process Event Use Case", () => {
  ...
  it("marks PROCESSING then COMPLETED and dispatches to the handler", async () => {
    ...
    globalExpect(handler.calls).toBe(1);
    globalExpect(markProcessing).toHaveBeenCalledWith(event.id);
    const stored = await eventsRepository.findById(event.id);
    globalExpect(stored?.status).toBe("COMPLETED");
    globalExpect(stored?.processed_at).toEqual(globalExpect.any(Date));
  });
  ...
  it("throws when no handler is registered for the event type", async () => {
    sut = new ProcessEventUseCase(eventsRepository, {});
    const event = await seedPendingEvent();
    await globalExpect(sut.execute({ eventId: event.id })).rejects.toThrow(
  ...
  it("on terminal handler failure: marks FAILED with attempts + last_error and re-throws", async () => {
    ...
    const stored = await eventsRepository.findById(event.id);
    globalExpect(stored?.status).toBe("FAILED");
    globalExpect(stored?.attempts).toBe(5);
    globalExpect(stored?.last_error).toBe("boom");
  });
  ...
  it("sweeper: findStalePending returns old PENDING rows for re-enqueue", async () => {
    const event = await seedPendingEvent();
    const stored = await eventsRepository.findById(event.id);
    if (stored) stored.created_at = /* @__PURE__ */ new Date("2000-01-01T00:00:00Z");
    const stale = await eventsRepository.findStalePending(/* @__PURE__ */ new Date());
    globalExpect(stale.map((e) => e.id)).toContain(event.id);
  });
});
