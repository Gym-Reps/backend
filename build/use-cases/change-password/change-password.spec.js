...
// src/use-cases/change-password/change-password.spec.ts
...
var InMemoryUsersRepository = class {
  ...
  async save(user) {
    const index2 = this.items.findIndex((item) => item.id === user.id);
    if (index2 >= 0) {
      this.items[index2] = user;
    }
    return user;
  }
};
...
describe("Change Password Use Case", () => {
  ...
  it("should be able to change the password and re-hash it", async () => {
    ...
    globalExpect(user.password_hash).not.toEqual("123456");
    globalExpect(await (0, import_bcryptjs2.compare)("new-password", user.password_hash)).toBe(true);
    globalExpect(await (0, import_bcryptjs2.compare)("123456", user.password_hash)).toBe(false);
  });
