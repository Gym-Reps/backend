...
var FetchUserTrainmentsUseCase = class {
  constructor(trainmentsRepository2) {
...
// src/use-cases/trainments/fetch-user-trainments/fetch-user-trainments.spec.ts
...
describe("Fetch User Trainments Use Case", () => {
  ...
  it("should list the user sessions newest first", async () => {
    await trainmentsRepository.create({
      ...
      user_id: "user-1",
      started_at: /* @__PURE__ */ new Date("2026-06-20T10:00:00Z")
    });
    await trainmentsRepository.create({
      trainment_template_id: "template-1",
      user_id: "user-1",
      started_at: /* @__PURE__ */ new Date("2026-06-25T10:00:00Z")
    });
    await trainmentsRepository.create({
      trainment_template_id: "template-1",
      user_id: "user-2",
      started_at: /* @__PURE__ */ new Date("2026-06-26T10:00:00Z")
    });
    const { trainments } = await sut.execute({ userId: "user-1", page: 1 });
    globalExpect(trainments).toHaveLength(2);
    globalExpect(trainments[0]?.started_at).toEqual(/* @__PURE__ */ new Date("2026-06-25T10:00:00Z"));
    globalExpect(trainments[1]?.started_at).toEqual(/* @__PURE__ */ new Date("2026-06-20T10:00:00Z"));
  });
  it("should filter by trainmentTemplateId", async () => {
    ...
    globalExpect(trainments).toHaveLength(1);
    globalExpect(trainments[0]?.trainment_template_id).toEqual("template-2");
  });
  it("should paginate 20 sessions per page", async () => {
    for (let i2 = 0; i2 < 22; i2++) {
      await trainmentsRepository.create({
        trainment_template_id: "template-1",
        user_id: "user-1",
        started_at: new Date(2026, 5, 1, i2)
      });
    }
    const page1 = await sut.execute({ userId: "user-1", page: 1 });
    const page2 = await sut.execute({ userId: "user-1", page: 2 });
    globalExpect(page1.trainments).toHaveLength(20);
    globalExpect(page2.trainments).toHaveLength(2);
  });
});
