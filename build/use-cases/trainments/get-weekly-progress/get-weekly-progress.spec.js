...
var SnapshotClient = class {
  ...
  async pollMatchDomain(options) {
    ...
    const reference2 = expectedSnapshot.data && snapshotState.snapshotUpdateState !== "all" ? adapter.parseExpected(expectedSnapshot.data) : void 0;
    ...
    const stableResult = await getStableSnapshot({
      ...
      interval,
      timedOut,
      match: reference2 ? (captured) => adapter.match(captured, reference2).pass : void 0
    });
    expectedSnapshot.markAsChecked();
...
var InMemoryUserPreferencesRepository = class {
  ...
  async save(preferences) {
    const index2 = this.items.findIndex((item) => item.id === preferences.id);
    if (index2 >= 0) {
      this.items[index2] = { ...preferences, updated_at: /* @__PURE__ */ new Date() };
...
function getWeekRange(reference2 = /* @__PURE__ */ new Date()) {
  const startOfDay = new Date(
    Date.UTC(
      reference2.getUTCFullYear(),
      reference2.getUTCMonth(),
      reference2.getUTCDate()
    )
  );
...
var GetWeeklyProgressUseCase = class {
  constructor(trainmentsRepository2, userPreferencesRepository2) {
  ...
  async execute({
    ...
    const { weekStart, weekEnd } = getWeekRange(reference2);
...
// src/use-cases/trainments/get-weekly-progress/get-weekly-progress.spec.ts
var trainmentsRepository;
var userPreferencesRepository;
var sut;
var reference = /* @__PURE__ */ new Date("2026-06-30T12:00:00Z");
describe("Get Weekly Progress Use Case", () => {
  ...
  it("should count only the user finished sessions inside the current week", async () => {
    await trainmentsRepository.create({
      ...
      user_id: "user-1",
      started_at: /* @__PURE__ */ new Date("2026-06-30T09:00:00Z"),
      finished_at: /* @__PURE__ */ new Date("2026-06-30T10:00:00Z")
    });
    await trainmentsRepository.create({
      trainment_template_id: "template-1",
      user_id: "user-1",
      started_at: /* @__PURE__ */ new Date("2026-07-01T09:00:00Z"),
      finished_at: /* @__PURE__ */ new Date("2026-07-01T10:00:00Z")
    });
    await trainmentsRepository.create({
      trainment_template_id: "template-1",
      user_id: "user-1",
      started_at: /* @__PURE__ */ new Date("2026-06-24T09:00:00Z"),
      finished_at: /* @__PURE__ */ new Date("2026-06-24T10:00:00Z")
    });
    await trainmentsRepository.create({
      trainment_template_id: "template-1",
      user_id: "user-1",
      started_at: /* @__PURE__ */ new Date("2026-07-02T09:00:00Z")
    });
    await trainmentsRepository.create({
    ...
    globalExpect(result.weekStart).toEqual(/* @__PURE__ */ new Date("2026-06-29T00:00:00.000Z"));
    globalExpect(result.weekEnd).toEqual(/* @__PURE__ */ new Date("2026-07-05T23:59:59.999Z"));
    globalExpect(result.completed).toEqual(2);
    globalExpect(result.goal).toBeNull();
    globalExpect(result.trainments).toHaveLength(2);
    globalExpect(result.trainments[0]?.finished_at).toEqual(
      /* @__PURE__ */ new Date("2026-07-01T10:00:00Z")
    );
  });
  it("should report zero completed when there are no finished sessions this week", async () => {
    const result = await sut.execute({ userId: "user-1", reference });
    globalExpect(result.completed).toEqual(0);
    globalExpect(result.trainments).toHaveLength(0);
    globalExpect(result.goal).toBeNull();
  });
  it("should return the goal from the user preferences when set", async () => {
    ...
    const result = await sut.execute({ userId: "user-1", reference });
    globalExpect(result.goal).toEqual(4);
  });
});
