...
var EnqueueEventUseCase = class {
  constructor(eventsRepository2, eventQueue2) {
...
var FinishTrainmentUseCase = class {
  constructor(trainmentsRepository2, enqueueEventUseCase) {
...
// src/use-cases/trainments/finish-trainment/finish-trainment.spec.ts
...
describe("Finish Trainment Use Case", () => {
  ...
  it("should set finished_at on an in-progress session", async () => {
    ...
    globalExpect(trainment.finished_at).toEqual(globalExpect.any(Date));
    globalExpect(eventsRepository.items).toHaveLength(1);
    globalExpect(eventsRepository.items[0]?.event_type).toBe(
      "COMPUTE_TRAINMENT_METRICS"
    );
    globalExpect(eventsRepository.items[0]?.metadata).toEqual({
      trainmentId: created.id
    });
    globalExpect(eventQueue.jobs).toHaveLength(1);
    globalExpect(eventQueue.jobs[0]?.eventId).toBe(eventsRepository.items[0]?.id);
  });
