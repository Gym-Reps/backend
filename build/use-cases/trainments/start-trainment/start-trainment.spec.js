...
var StartTrainmentUseCase = class {
  constructor(trainmentsRepository2, trainmentTemplatesRepository2) {
...
// src/use-cases/trainments/start-trainment/start-trainment.spec.ts
...
describe("Start Trainment Use Case", () => {
  ...
  it("should start a session from an owned template with finished_at null", async () => {
    ...
    globalExpect(trainment.id).toEqual(globalExpect.any(String));
    globalExpect(trainment.started_at).toEqual(globalExpect.any(Date));
    globalExpect(trainment.finished_at).toBeNull();
    globalExpect(trainment.trainment_template_id).toEqual(template.id);
    globalExpect(trainmentsRepository.items).toHaveLength(1);
  });
  ...
  it("should reject starting from a soft-deleted template but keep past trainments resolvable", async () => {
    ...
    template.deleted_at = /* @__PURE__ */ new Date();
