...
var FetchUserTrainmentTemplatesUseCase = class {
  constructor(trainmentTemplatesRepository2) {
...
// src/use-cases/trainment-templates/fetch-user-trainment-templates/fetch-user-trainment-templates.spec.ts
...
describe("Fetch User Trainment Templates Use Case", () => {
  ...
  it("should return only the caller's active templates", async () => {
    ...
    globalExpect(trainmentTemplates).toHaveLength(2);
    globalExpect(trainmentTemplates).toEqual([
      globalExpect.objectContaining({ title: "Upper A" }),
      globalExpect.objectContaining({ title: "Lower B" })
    ]);
  });
  it("should exclude soft-deleted templates", async () => {
    ...
    deleted.deleted_at = /* @__PURE__ */ new Date();
    ...
    const { trainmentTemplates } = await sut.execute({ userId: "user-1" });
    globalExpect(trainmentTemplates).toHaveLength(1);
    globalExpect(trainmentTemplates[0]?.title).toEqual("Upper A");
  });
});
