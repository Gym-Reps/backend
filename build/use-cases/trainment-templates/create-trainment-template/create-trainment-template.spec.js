...
// src/use-cases/trainment-templates/create-trainment-template/create-trainment-template.spec.ts
...
describe("Create Trainment Template Use Case", () => {
  ...
  it("should be able to create a trainment template for the user", async () => {
    ...
    globalExpect(trainmentTemplate.id).toEqual(globalExpect.any(String));
    globalExpect(trainmentTemplate.title).toEqual("Upper A");
    globalExpect(trainmentTemplate.user_id).toEqual("user-1");
    globalExpect(trainmentTemplate.deleted_at).toBeNull();
    globalExpect(trainmentTemplatesRepository.items).toHaveLength(1);
  });
});
