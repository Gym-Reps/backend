...
var SearchExercisesUseCase = class {
  constructor(defaultExercisesRepository2) {
...
// src/use-cases/default-exercises/search-exercises/search-exercises.spec.ts
...
describe("Search Exercises Use Case", () => {
  ...
  it("should match by partial title, case-insensitively", async () => {
    ...
    globalExpect(total).toEqual(1);
    globalExpect(exercises).toHaveLength(1);
    globalExpect(exercises[0]?.title).toEqual("Barbell Bench Press");
  });
  it("should filter by muscle group", async () => {
    ...
    globalExpect(total).toEqual(1);
    globalExpect(exercises[0]?.title).toEqual("Pull Up");
  });
  it("should paginate, 20 per page", async () => {
    for (let i2 = 0; i2 < 22; i2++) {
      await defaultExercisesRepository.create({
        ...
        slug: `exercise-${i2}`,
        muscle_group: "CORE",
        image_path: `/static/exercises/exercise-${i2}.webp`
      });
    }
    const page1 = await sut.execute({ page: 1 });
    const page2 = await sut.execute({ page: 2 });
    globalExpect(page1.exercises).toHaveLength(20);
    globalExpect(page1.total).toEqual(22);
    globalExpect(page2.exercises).toHaveLength(2);
    globalExpect(page2.total).toEqual(22);
  });
});
