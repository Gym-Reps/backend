...
// src/use-cases/default-exercises/create-exercise/create-exercise.spec.ts
...
describe("Create Exercise Use Case", () => {
  ...
  it("should create an entry, deriving the slug from the title", async () => {
    ...
    globalExpect(exercise.id).toEqual(globalExpect.any(String));
    globalExpect(exercise.slug).toEqual("barbell-bench-press");
    globalExpect(defaultExercisesRepository.items).toHaveLength(1);
  });
  it("should honor an explicit slug", async () => {
    ...
    globalExpect(exercise.slug).toEqual("bench");
  });
