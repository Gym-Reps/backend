...
var RemoveExerciseFromTrainmentUseCase = class {
  constructor(exercisesRepository2, trainmentsRepository2, setsRepository2) {
...
// src/use-cases/exercises/remove-exercise-from-trainment/remove-exercise-from-trainment.spec.ts
...
describe("Remove Exercise From Trainment Use Case", () => {
  ...
  it("deletes the exercise and its sets", async () => {
    ...
    globalExpect(await exercisesRepository.findById(exercise.id)).toBeNull();
    globalExpect(await setsRepository.countByExerciseId(exercise.id)).toEqual(0);
  });
  it("throws NotAllowedError when owned by another user", async () => {
    ...
    globalExpect(await setsRepository.countByExerciseId(exercise.id)).toEqual(2);
  });
});
