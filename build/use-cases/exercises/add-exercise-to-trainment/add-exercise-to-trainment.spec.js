...
var InMemoryTrainmentsRepository = class {
  ...
  async create(data) {
    const trainment = {
      id: data.id ?? (0, import_node_crypto5.randomUUID)(),
      trainment_template_id: data.trainment_template_id,
      user_id: data.user_id,
...
function plannedSetIndices(plannedSets2) {
  return Object.keys(plannedSets2).map(Number).sort((a, b2) => a - b2);
}

// src/use-cases/sets/create-sets-for-exercise/create-sets-for-exercise.ts
var CreateSetsForExerciseUseCase = class {
  constructor(setsRepository2) {
  ...
  async execute({
    ...
    trainmentId,
    exerciseId,
    plannedSets: plannedSets2
  }) {
    const sets = await this.setsRepository.createMany(
      plannedSetIndices(plannedSets2).map((index2) => ({
...
var AddExerciseToTrainmentUseCase = class {
  constructor(exercisesRepository2, trainmentsRepository2, exerciseTemplatesRepository2, trainmentTemplatesRepository2, createSetsForExerciseUseCase) {
...
// src/use-cases/exercises/add-exercise-to-trainment/add-exercise-to-trainment.spec.ts
...
describe("Add Exercise To Trainment Use Case", () => {
  ...
  it("creates the exercise with planned_sets and materializes the matching sets", async () => {
    ...
    globalExpect(exercise.planned_sets).toEqual(plannedSets);
    globalExpect(sets).toHaveLength(3);
    globalExpect(sets.map((s) => s.index)).toEqual([1, 2, 3]);
    globalExpect(await setsRepository.countByExerciseId(exercise.id)).toEqual(3);
  });
