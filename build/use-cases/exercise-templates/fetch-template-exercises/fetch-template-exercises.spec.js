...
// src/use-cases/exercise-templates/fetch-template-exercises/fetch-template-exercises.spec.ts
...
describe("Fetch Template Exercises Use Case", () => {
  ...
  it("lists active slots and excludes soft-deleted ones", async () => {
    ...
    removed.deleted_at = /* @__PURE__ */ new Date();
    await exerciseTemplatesRepository.save(removed);
    const { exerciseTemplates } = await sut.execute({
      userId: "user-1",
      trainmentTemplateId: template.id
    });
    globalExpect(exerciseTemplates).toHaveLength(1);
    globalExpect(exerciseTemplates[0]?.title).toEqual("Squat");
  });
