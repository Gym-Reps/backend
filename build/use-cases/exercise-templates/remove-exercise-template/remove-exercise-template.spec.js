...
var RemoveExerciseTemplateUseCase = class {
  constructor(exerciseTemplatesRepository2, trainmentTemplatesRepository2) {
...
// src/use-cases/exercise-templates/remove-exercise-template/remove-exercise-template.spec.ts
...
describe("Remove Exercise Template Use Case", () => {
  ...
  it("soft-deletes the slot, bumps the parent updated_at, and a past exercise still resolves it", async () => {
    ...
    template.updated_at = /* @__PURE__ */ new Date("2020-01-01");
    ...
    globalExpect(await exerciseTemplatesRepository.findById(slot.id)).toBeNull();
    globalExpect(
    ...
    globalExpect(parent.updated_at.getTime()).toBeGreaterThan(
      (/* @__PURE__ */ new Date("2020-01-01")).getTime()
    );
    const stillThere = await exercisesRepository.findById(performed.id);
    globalExpect(stillThere?.exercise_template_id).toEqual(slot.id);
  });
