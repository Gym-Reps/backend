...
async function addSetToExercise(request, reply) {
  const paramsSchema = import_zod2.z.object({ id: import_zod2.z.uuid() });
  const bodySchema = import_zod2.z.object({
    weight: import_zod2.z.number().optional(),
    minReps: import_zod2.z.number().int().optional(),
    maxReps: import_zod2.z.number().int().optional()
  });
...
async function fetchSetsByExercise(request, reply) {
  const paramsSchema = import_zod3.z.object({ id: import_zod3.z.uuid() });
  const { id } = paramsSchema.parse(request.params);
  try {
...
async function fetchSetsByTrainment(request, reply) {
  const paramsSchema = import_zod4.z.object({ id: import_zod4.z.uuid() });
  const { id } = paramsSchema.parse(request.params);
  try {
...
async function updateSet(request, reply) {
  ...
  const bodySchema = import_zod6.z.object({
    weight: import_zod6.z.number().min(0).optional(),
    repetitions: import_zod6.z.number().int().min(0).optional(),
    performedAt: import_zod6.z.coerce.date().optional()
