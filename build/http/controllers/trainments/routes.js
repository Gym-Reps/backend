...
__export(routes_exports, {
  trainmentRoutes: () => trainmentRoutes
});
...
async function fetchUserTrainments(request, reply) {
  const fetchUserTrainmentsQuerySchema = import_zod2.z.object({
    trainmentTemplateId: import_zod2.z.uuid().optional(),
...
async function finishTrainment(request, reply) {
  const finishTrainmentParamsSchema = import_zod3.z.object({
...
async function getTrainment(request, reply) {
  const getTrainmentParamsSchema = import_zod4.z.object({
...
async function startTrainment(request, reply) {
  const startTrainmentBodySchema = import_zod5.z.object({
    trainmentTemplateId: import_zod5.z.uuid()
  });
...
var plannedSetsSchema = import_zod6.z.record(
  import_zod6.z.string().regex(/^\d+$/),
  import_zod6.z.object({
    weight: import_zod6.z.number().nullable(),
    min_reps: import_zod6.z.number().int().nullable(),
    max_reps: import_zod6.z.number().int().nullable()
  })
).refine(
  (plannedSets) => {
...
// src/http/controllers/trainments/sync-trainment/sync-trainment.ts
var setSchema = import_zod7.z.object({
  id: import_zod7.z.uuid(),
  index: import_zod7.z.number().int().min(1),
  weight: import_zod7.z.number().min(0).nullable(),
  repetitions: import_zod7.z.number().int().min(0).nullable(),
  performedAt: import_zod7.z.coerce.date()
});
var exerciseSchema = import_zod7.z.object({
  id: import_zod7.z.uuid(),
  exerciseTemplateId: import_zod7.z.uuid(),
  plannedSets: plannedSetsSchema,
  sets: import_zod7.z.array(setSchema)
...
var syncTrainmentBodySchema = import_zod7.z.object({
  id: import_zod7.z.uuid(),
  trainmentTemplateId: import_zod7.z.uuid(),
  startedAt: import_zod7.z.coerce.date(),
  finishedAt: import_zod7.z.coerce.date().nullable(),
  exercises: import_zod7.z.array(exerciseSchema)
});
...
// src/http/controllers/trainments/routes.ts
async function trainmentRoutes(app) {
