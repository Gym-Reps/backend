...
var envSchema = import_zod2.z.object({
  NODE_ENV: import_zod2.z.enum(["dev", "test", "prod"]).default("dev"),
  PORT: import_zod2.z.coerce.number().default(3333),
  DATABASE_URL: import_zod2.z.url(),
  SECRET_KEY: import_zod2.z.string(),
  APP_URL: import_zod2.z.url().default("http://localhost:3333"),
  ...
  REDIS_URL: import_zod2.z.url().default("redis://localhost:6379")
});
...
if (!_env.success) {
  console.error(`Invalid environment variables, ${import_zod2.z.treeifyError(_env.error)}`);
...
// src/http/controllers/trainments/sync-trainment/sync-trainment.ts
var setSchema = import_zod3.z.object({
  id: import_zod3.z.uuid(),
  index: import_zod3.z.number().int().min(1),
  weight: import_zod3.z.number().min(0).nullable(),
  repetitions: import_zod3.z.number().int().min(0).nullable(),
  performedAt: import_zod3.z.coerce.date()
});
var exerciseSchema = import_zod3.z.object({
  id: import_zod3.z.uuid(),
  exerciseTemplateId: import_zod3.z.uuid(),
  plannedSets: plannedSetsSchema,
  sets: import_zod3.z.array(setSchema)
...
var syncTrainmentBodySchema = import_zod3.z.object({
  id: import_zod3.z.uuid(),
  trainmentTemplateId: import_zod3.z.uuid(),
  startedAt: import_zod3.z.coerce.date(),
  finishedAt: import_zod3.z.coerce.date().nullable(),
  exercises: import_zod3.z.array(exerciseSchema)
});
